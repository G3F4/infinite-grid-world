...
var DynamicTerrain = /** @class */ (function () {
  ...
  function DynamicTerrain(name, options, scene) {
    ...
    if (this._mapInstanceData) {
      ...
      for (var t = 0; t < typeNb; t++) {
        ...
        var nb = mesh.instances.length as number;
        ...
        if (this._colorInstanceData) {
          ...
          var colorBuffer = new VertexBuffer(engine, colorArray, VertexBuffer.ColorKind, true, false, 4, true);
   ...
   * If the optional object {normal: Vector3} is passed, then its property "normal" is updated with the normal vector value at the coordinates (x, z).
   */
  DynamicTerrain.GetHeightFromMap = function (x, z, mapData, mapSubX, mapSubZ, options, inverted = false) {
  ...
  DynamicTerrain._pos = Vector3.Zero();
  DynamicTerrain._scl = Vector3.Zero();
  // tmp quaternion and matrix or arrays
  DynamicTerrain._quat = Quaternion.Identity();
